...
class SettingsPane(QWidget):
    ...
    def _terminal_help_html(self) -> str:
        return (
            "<b>Terminal commands</b> (headless — same codebase, single-instance "
            "lock means CLI + GUI can't run at the same time):<br>"
            "&nbsp;• <b>add &lt;name-or-rss-or-youtube-url&gt;</b> — adds a show "
            "(podcast by iTunes name / RSS URL, or YouTube channel/handle URL — "
            "yt-dlp lazy-installs on first YouTube use); seeds episodes as pending<br>"
            "&nbsp;• <b>list</b> — prints the watchlist with the source column "
            "(podcast / youtube)<br>"
            "&nbsp;• <b>check [--show &lt;slug&gt;] [--limit N]</b> — refresh feeds "
            "+ drain queue. YouTube tries captions first (requested lang → en → "
            "any) then falls back to whisper. Stays useful offline: feeds + new "
            "downloads fail fast, but already-downloaded MP3s keep transcribing.<br>"
            "&nbsp;• <b>import-feeds</b> — bulk-imports the curated podcast list<br><br>"
            "Run from <code>~/dev/paragraphos/</code>:<br>"
            "<code>PYTHONPATH=. .venv/bin/python cli.py &lt;cmd&gt;</code><br><br>"
            "<b>Logs:</b> every launch writes a one-line fingerprint to "
            "<code>~/Library/Logs/Paragraphos/paragraphos.log</code> covering "
            "version, OS, RAM/cores, whisper-cli + yt-dlp + ffmpeg versions, and "
            "every user-tunable setting (with <code>(rec=N)</code> hints when "
            "your parallel/multiproc differs from the hardware-aware "
            "recommendation). Grep that line first for support."
        )
    ...
    def _agent_prompt_plain(self) -> str:
        return (
            ...
            "side; YouTube tries uploader captions first (requested language → en\n"
            "→ any available) and falls back to whisper-cli when none are usable.\n"
            "yt-dlp is lazy-installed to\n"
            "  ~/Library/Application Support/Paragraphos/bin/yt-dlp\n"
            "on first YouTube use, and self-updates weekly.\n"
            "\n"
            "Single-instance lock at\n"
            "  ~/Library/Application Support/Paragraphos/paragraphos.lock\n"
            "means the CLI and the GUI can't run concurrently — quit the app\n"
            "first, or run the CLI when the GUI is closed.\n"
            "\n"
            "State (shows, queue, completed episodes, settings):\n"
            ...
            "\n"
            "Logs (one-line startup fingerprint per launch — version, OS, RAM,\n"
            "cores, whisper-cli + yt-dlp + ffmpeg versions, and every user-\n"
            "tunable setting, with `(rec=N)` annotations when current values\n"
            "differ from the hardware recommendation):\n"
            "  ~/Library/Logs/Paragraphos/paragraphos.log\n"
            "\n"
            "Settings of interest (settings.yaml):\n"
            "  sources_podcasts: true             # include RSS podcasts\n"
            "  sources_youtube:  true             # include YouTube channels\n"
            "  youtube_default_transcript_source: captions  # captions | whisper | auto-captions\n"
            "  youtube_default_language: de       # caption pref; falls back to en, then any\n"
            "  parallel_transcribe: 1             # how many whisper-cli runs in parallel\n"
            "  whisper_multiproc: 1               # whisper-cli -p N split for one file\n"
            "  whisper_fast_mode: false           # beam=1/best=1, ~2-3× faster, lower quality\n"
            "  save_srt: true                     # also write .srt next to .md\n"
            "  mp3_retention_days: 7              # how long to keep MP3s after transcribe\n"
            "  auto_start_queue: true             # auto-drain queue on launch\n"
            "  auto_start_delay_seconds: 5        # grace period before auto-drain\n"
            "  connectivity_monitor_enabled: true # banner + auto-resume on reconnect\n"
            "  auto_resume_failed_window_hours: 24  # re-queue network-failed eps from last N h\n"
            "  ytdlp_last_self_update_at: ''      # ISO timestamp; weekly auto-update\n"
            "\n"
            ...
            "\n"
            "Commands:\n"
            "  add <name-or-url>         — add a show. Accepts:\n"
            "                              · podcast: iTunes search name or RSS URL\n"
            "                              · YouTube: channel URL (https://youtube.com/@handle\n"
            "                                or /channel/UC...) — yt-dlp auto-installs if missing\n"
            "  list                      — print the watchlist (source column = 'podcast'/'youtube')\n"
            "  check [--show <slug>] [--limit N]\n"
            "                            — refresh feeds + process pending episodes.\n"
            "                              YouTube items try captions first, then whisper.\n"
            "                              Works offline too: feed-fetch and new downloads\n"
            "                              fail fast, but already-downloaded MP3s keep\n"
            "                              transcribing locally.\n"
            "  import-feeds              — bulk-import the curated podcast list\n"
            "\n"
            "GUI-only operations not yet in the CLI (edit watchlist.yaml or sqlite\n"
            "directly): pause/stop queue, run next / run now (priority bump),\n"
            "re-transcribe, remove a show, clear queue, toggle show enabled.\n"
            "\n"
            "Query state directly with sqlite (read-only is safe while GUI runs;\n"
            "writes need the GUI closed because of the single-instance lock):\n"
            "  sqlite3 ~/Library/Application\\ Support/Paragraphos/state.sqlite \\\n"
            '          "SELECT status, COUNT(*) FROM episodes GROUP BY status;"\n'
            "\n"
            "Bump an episode's priority (== 'run next' from the GUI):\n"
            "  sqlite3 ~/Library/Application\\ Support/Paragraphos/state.sqlite \\\n"
            "          \"UPDATE episodes SET priority=100 WHERE guid='<guid>';\"\n"
            "\n"
            "Mark an episode for re-transcription:\n"
            "  sqlite3 ~/Library/Application\\ Support/Paragraphos/state.sqlite \\\n"
            "          \"UPDATE episodes SET status='pending', priority=100 \\\n"
            "           WHERE guid='<guid>';\"\n"
            "\n"
            "Per-show overrides (watchlist.yaml):\n"
            "  - slug: my-channel\n"
            "    source: youtube\n"
            "    enabled: true                    # set false to skip in checks\n"
            "    language: en                     # whisper lang code; 'auto' = detect\n"
            "    youtube_transcript_pref: whisper # force whisper; ignore captions\n"
            "    whisper_prompt: '...'            # bias domain vocabulary\n"
            "    output_override: '/some/path'    # custom transcript dir\n"
            "\n"
            "Example tasks you can give an agent:\n"
            "  · 'Add the podcast X and the YouTube channel Y, then transcribe the\n"
            "     latest 5 episodes of each.'\n"
            "  · 'Switch all YouTube shows to always-whisper mode and re-run check\n"
            "     for the last 10 episodes.'\n"
            "  · 'Disable the YouTube source temporarily (sources_youtube: false),\n"
            "     run a podcast-only check pass, then re-enable it.'\n"
            "  · 'List every YouTube episode that fell back to whisper because no\n"
            "     captions were available.'\n"
            "  · 'Show me the last startup fingerprint from the log and tell me if\n"
            "     my parallel/multiproc settings match the recommendation.'\n"
            "  · 'Bump the 3 newest episodes of show <slug> to the front of the\n"
            "     queue, then run check.'\n"
            "\n"
