...
class SettingsPane(QWidget):
    ...
    def _terminal_help_html(self) -> str:
        return (
            ...
            "&nbsp;&nbsp;&nbsp;A YouTube channel or <code>@handle</code> URL is "
            "auto-detected (<code>source=youtube</code>): new uploads are picked up "
            "from the channel feed, then each video is imported from the uploader's "
            "own subtitles when present (manual only — auto-generated captions are "
            "never used) and falls back to whisper otherwise. In the GUI, the Shows "
            "tab has a dedicated <b>Add YouTube Channel…</b> button: paste the link, "
            "edit the slug (defaults to the channel name), pick a backfill window "
            "(only-new / last&nbsp;5/20/100 / since a date), and toggle caption "
            "import.<br>"
    ...
    def _agent_prompt_plain(self) -> str:
        return (
            ...
            "                                       A YouTube channel/@handle URL is\n"
            "                                       auto-detected → source=youtube\n"
            "                                       (GUI: dedicated 'Add YouTube\n"
            "                                       Channel…' button on the Shows tab).\n"
