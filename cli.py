...
def _state() -> StateStore:
    s = StateStore(DATA / "state.sqlite"); s.init_schema(); return s
...
def cmd_add(args: argparse.Namespace) -> int:
    ...
    prompt = suggest_whisper_prompt(
        ...
        episodes=[{"title": e["title"], "description": e["description"]}
                  for e in manifest[-20:]],  # most-recent 20
    )
    ...
    if any(s.slug == slug for s in wl.shows):
        print(f"show {slug!r} already in watchlist"); return 3
    wl.shows.append(Show(slug=slug, title=meta["title"], rss=rss,
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    lib = LibraryIndex(out_root); lib.scan()

    targets = [s for s in wl.shows if s.enabled and
               (not args.show or s.slug == args.show)]
    if not targets:
        print("no enabled shows match"); return 0
    ...
    for show in targets:
        ...
        for ep in manifest:
            state.upsert_episode(show_slug=show.slug, guid=ep["guid"],
                                 title=ep["title"], pub_date=ep["pubDate"],
                                 mp3_url=ep["mp3_url"])
        ...
        if not pending:
            print("  no pending"); continue

        ctx = PipelineContext(
            state=state, library=lib, output_root=out_root,
        ...
        for ep in pending:
            r = process_episode(ep["guid"], ctx,
                                episode_number=ep_num_map.get(ep["guid"], "0000"))
...
def cmd_import_feeds(args: argparse.Namespace) -> int:
    ...
    for slug, rss in feeds:
        if slug in existing:
            print(f"skip {slug} (already in watchlist)"); continue
        try:
            meta = feed_metadata(rss)
        ...
        wl.shows.append(Show(slug=slug, title=meta["title"] or slug,
                             rss=rss, whisper_prompt=prompt))
        for ep in manifest:
            state.upsert_episode(show_slug=slug, guid=ep["guid"], title=ep["title"],
                                 pub_date=ep["pubDate"], mp3_url=ep["mp3_url"])
...
def main() -> int:
    ...
    a = sub.add_parser("add"); a.add_argument("name_or_url"); a.set_defaults(fn=cmd_add)
